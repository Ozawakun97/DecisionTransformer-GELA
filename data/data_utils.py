...
class ImageFeaturesDB(object):
    def __init__(self, img_ft_file, image_feat_size, img_aug_ft_file=None):
        self.image_feat_size = image_feat_size
        self.img_ft_file = img_ft_file
        self.img_aug_ft_file = img_aug_ft_file
        self._feature_store = {}
        if img_aug_ft_file:
            self._feature_store_aug = {}
        else:
            self._feature_store_aug = None

    def get_image_feature(self, scan, viewpoint):
        key = '%s_%s' % (scan, viewpoint)
        if self.img_aug_ft_file:
            ran = random.random()
            if ran > 0.5:
                if key in self._feature_store_aug:
                    fts = self._feature_store_aug[key]
                else:
                    with h5py.File(self.img_aug_ft_file, 'r') as f1:
                        fts = f1[key][...].astype(np.float32)[:,:self.image_feat_size]
                    self._feature_store_aug[key] = fts
            else:
                if key in self._feature_store:
                    fts = self._feature_store[key]
                else:
                    with h5py.File(self.img_ft_file, 'r') as f2:
                        fts = f2[key][...].astype(np.float32)[:,:self.image_feat_size]
            ...
            if key in self._feature_store:
                ...
                with h5py.File(self.img_ft_file, 'r') as f:
                    fts = f[key][...].astype(np.float32)[:,:self.image_feat_size]
                self._feature_store[key] = fts
        return fts
...
def load_instr_datasets(anno_dir, dataset, splits):
    ...
    for split in splits:
        if "/" not in split:    # the official splits
            if dataset == 'r2r':
                with open(os.path.join(anno_dir, 'R2R_%s_seq.jsonl' % split)) as f:
                    new_data = []
                    for line in f:    
...
def load_nav_graphs(connectivity_dir, scans = None):
    ...
    graphs = {}
    if scans is None:
        scans = glob.glob(f"{connectivity_dir}/*_connectivity.json")
    for scan in scans:
