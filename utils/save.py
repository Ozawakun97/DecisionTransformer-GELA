...
class DecisionTransformerSaver(object):
    def __init__(self, output_dir, prefix='model_step', suffix='pth'):
        self.output_dir = output_dir
        self.prefix = prefix
        self.suffix = suffix
        self.record_dict = {}
        self.val_dict = {}
    def save(self, model,optimizer,scheduler,val_type,loss):
        if val_type in self.val_dict and self.val_dict[val_type] is not None:
            if abs(loss - self.val_dict[val_type]) < 0.01:
                return
        self.val_dict[val_type] = loss
        if val_type in self.record_dict and self.record_dict[val_type] is not None:
            if os.path.exists(self.record_dict[val_type]):
                os.remove(self.record_dict[val_type])
        output_model_file = os.path.join(self.output_dir,
                                 f"{val_type}={loss:.2f}.{self.suffix}")
        ...
        torch.save(state_dict, output_model_file)
        self.record_dict[val_type] = output_model_file
